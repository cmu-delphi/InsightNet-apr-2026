...
> **Hint:** use `.facet_by = "geo_value"` to view each state in its own panel.

```{r}
#| label: sliding-plot
autoplot(edf, value, value_8wav, .facet_by = "geo_value")
```
...
mod_rt <- cv_estimate_rt(observed_counts = edf_mi_rt$value)
...
archive_raw <- pub_covidcast(
  ...
  time_values = epirange(20200101, 20210601),
  issues = "*"
)
```
...
autoplot(archive_data, value, .versions = "month")
```
...
```{r}
#| label: as-of
snapshot_early <- epix_as_of(archive_data, version = as.Date("2021-06-01"))
```
